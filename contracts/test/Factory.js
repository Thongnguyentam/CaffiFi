...
describe("Factory", function () {
  ...
  async function buyTokenFixture() {
    const { factory, token, liquidityPool, deployer, creator, buyer } =
  ...
  describe("Creating", function () {
    ...
    it("Should set the specified creator when real address is passed", async function () {
      const [deployer, creator, buyer, specifiedCreator] =
  ...
  describe("Buying", function () {
    ...
    it("should update user contribution", async function () {
      ...
      const tokenBalance = await factory.userTokenContributions(
        await token.getAddress(),
        buyer.address
      );
      const ethBalance = await factory.userEthContributions(
        await token.getAddress(),
        buyer.address
      );
      const [contributors, contributions] = await factory.getContributors(
        await token.getAddress()
      );
      ...
      const buyTx = await factory
        .connect(buyer)
        .buy(await token.getAddress(), AMOUNT / 2n, { value: COST });
      await buyTx.wait();
      const tokenBalance2 = await factory.userTokenContributions(
        await token.getAddress(),
        buyer.address
      );
      const ethBalance2 = await factory.userEthContributions(
        await token.getAddress(),
        buyer.address
      );
      const [contributors2, contributions2] = await factory.getContributors(
        await token.getAddress()
      );
      ...
      const buyTx2 = await factory
        ...
        .buy(await token.getAddress(), AMOUNT / 2n, { value: COST });
      await buyTx2.wait();
      const tokenBalance3 = await factory.userTokenContributions(
        await token.getAddress(),
        creator.address
      );
      const ethBalance3 = await factory.userEthContributions(
        await token.getAddress(),
        creator.address
      );
      const [contributors3, contributions3] = await factory.getContributors(
        await token.getAddress()
      );
  ...
  describe("Depositing", function () {
    ...
    it("Sale should be closed and successfully deposits", async function () {
      const { factory, token, liquidityPool, deployer, creator, buyer } =
        await loadFixture(buyTokenFixture);
      ...
      const userLiquidity = await liquidityPool.userLiquidity(
        await token.getAddress(),
        buyer.address
      );
    ...
    it("should have 2 contributors in liquidity pool", async function () {
      ...
      const buyTx2 = await factory
        .connect(creator)
        .buy(await token.getAddress(), HALF_AMOUNT, { value: HALF_COST });
      await buyTx2.wait();
      const buyTx = await factory
        .connect(buyer)
        .buy(await token.getAddress(), HALF_AMOUNT, { value: HALF_COST });
      ...
      // Check liquidity pool balances
      const liquidity = await liquidityPool.liquidity(await token.getAddress());
      const buyer1Liquidity = await liquidityPool.userLiquidity(
        await token.getAddress(),
        buyer.address
      );
      const buyer2Liquidity = await liquidityPool.userLiquidity(
        await token.getAddress(),
        creator.address
      );
      expect(liquidity).to.equal(ethers.parseUnits("3", 18));
  ...
  describe("Claiming Reward", function () {
    ...
    it("should claim rewards and cannot claim twice", async function () {
      ...
      expect(
        await factory.hasClaimedReward(await token.getAddress(), buyer.address)
      );
      await expect(
      ...
      const claimRewardTx = await factory
        .connect(buyer)
        .claimReward(await token.getAddress());
      ...
      const expectedBalance =
        ethers.parseUnits(rewards.toString(), 0) + AMOUNT * 2n;
      expect(ethers.parseUnits(tokenBalance.toString(), 0)).to.equal(
        expectedBalance
      );
    });
  });
  ...
  describe("Get Token Price", function () {
    ...
    it("should get token price", async function () {
      const { factory, token, buyer } = await loadFixture(buyTokenFixture);
      const tokenPrice = await factory.getPriceForTokens(
        await token.getAddress(),
        AMOUNT
      );
  ...
  describe("Swap", function () {
    const AMOUNT = ethers.parseUnits("5000", 18); // swap 5000 tokens

    it("Should burn tokens when swapped", async function () {
      const { factory, token, buyer } = await loadFixture(buyTokenFixture);

      // Approve tokens first
      await token.connect(buyer).approve(await factory.getAddress(), AMOUNT);
      ...
      // Perform swap
      await factory.connect(buyer).swap(await token.getAddress(), AMOUNT);
    ...
    it("Should emit Burned event", async function () {
      ...
      await expect(
        factory.connect(buyer).swap(await token.getAddress(), AMOUNT)
      )
        .to.emit(factory, "Burned")
        .withArgs(buyer.address, await token.getAddress(), AMOUNT);
    });
    ...
    it("Should revert if amount is zero", async function () {
      ...
      await expect(
        factory.connect(buyer).swap(await token.getAddress(), 0)
      ).to.be.revertedWith("Amount must be greater than 0");
    });

    it("Should revert if user has insufficient balance", async function () {
      const { factory, token, buyer } = await loadFixture(buyTokenFixture);
      const LARGE_AMOUNT = ethers.parseUnits("20000", 18); // more than user has

      await expect(
        factory.connect(buyer).swap(await token.getAddress(), LARGE_AMOUNT)
...
describe("NativeLiquidityPool", function () {
  ...
  async function deployLiquidityPoolFixture() {
    const [deployer, creator, contributor, contributor2, contributor3] =
      await ethers.getSigners();
    const Factory = await ethers.getContractFactory("Factory");
    const factory = await Factory.deploy(FEE);
  ...
  async function addLiquidityFixture() {
    ...
    const transaction2 = await factory
      .connect(contributor2)
      .buy(await token.getAddress(), AMOUNT / 2n, { value: COST });
    await transaction2.wait();

    const transaction3 = await factory
  ...
  describe("Liquidity Operations", function () {
    ...
    it("Should allow users to remove liquidity and receive ETH and tokens", async function () {
      const { factory, token, liquidityPool, deployer, creator, contributor } =
        await loadFixture(deployLiquidityPoolFixture);

      await expect(
        liquidityPool
          .connect(deployer)
          .removeLiquidity(await token.getAddress(), liquidityAmount)
    ...
    it("remove liquidity", async function () {
      ...
      await liquidityPool
        .connect(contributor)
        .removeLiquidity(await token.getAddress(), liquidityAmount);
      const remainingLiquidity = await liquidityPool.liquidity(
        await token.getAddress()
      );
      expect(remainingLiquidity).to.equal(COST * 2n);
      expect(
        await liquidityPool.userLiquidity(
          await token.getAddress(),
          contributor.address
        )
      ).to.equal(0);
      ...
      await liquidityPool
        .connect(contributor2)
        .removeLiquidity(await token.getAddress(), liquidityAmount);
      const remainingLiquidity2 = await liquidityPool.liquidity(
        await token.getAddress()
      );
      ...
      await liquidityPool
        .connect(contributor3)
        .removeLiquidity(await token.getAddress(), liquidityAmount);
      const remainingLiquidity3 = await liquidityPool.liquidity(
        await token.getAddress()
      );
    ...
    it("should allow users to remove partial liquidity", async function () {
      ...
      const liquidityBalance = await liquidityPool.liquidity(
        await token.getAddress()
      );
      ...
      await liquidityPool
        .connect(contributor)
        .removeLiquidity(await token.getAddress(), liquidityAmount / 2n);
      const remainingLiquidity = await liquidityPool.liquidity(
        await token.getAddress()
      );
      ...
      ).to.equal(liquidityAmount / 2n);
      expect(await token.balanceOf(contributor)).to.equal(
        tokenUserReceive + AMOUNT
      );
    });
    it("swap eth for tokens", async function () {
      ...
      expectedBalance =
        initBalance + (tokenReserve * ethAmount) / (ethReserve + ethAmount);
      await liquidityPool
        .connect(contributor)
        .swapEthForToken(await token.getAddress(), { value: ethAmount });
      expect(await token.balanceOf(contributor.address)).to.equal(
        expectedBalance
      );
    });

    it("swap tokens for eth", async function () {
      const { token, liquidityPool, contributor } = await loadFixture(
        addLiquidityFixture
      );
      ...
      await token
        .connect(contributor)
        .approve(await liquidityPool.getAddress(), tokenAmount);

      initBalance = await ethers.provider.getBalance(contributor.address);
      tokenReserve = await token.balanceOf(await liquidityPool.getAddress());
      ethReserve = await liquidityPool.liquidity(await token.getAddress());
      const expectedEthOut =
        (tokenAmount * ethReserve) / (tokenReserve + tokenAmount);
      console.log("expectedEthOut", expectedEthOut);
      await liquidityPool
        .connect(contributor)
        .swapTokenForEth(await token.getAddress(), tokenAmount);
    ...
    it("get estimated token for eth", async function () {
      ...
      const expectedTokenOut =
        (ethAmount * tokenReserve) / (ethReserve + ethAmount);
      const estimatedTokenOut = await liquidityPool.getEstimatedTokensForEth(
        await token.getAddress(),
        ethAmount
      );
    ...
    it("get estimated eth for token", async function () {
      ...
      const expectedEthOut =
        (tokenAmount * ethReserve) / (tokenReserve + tokenAmount);
      const estimatedEthOut = await liquidityPool.getEstimatedEthForTokens(
        await token.getAddress(),
        tokenAmount
      );
...
describe("LaunchpadAgent", function () {
  ...
  async function deployLaunchpadAgentFixture() {
    ...
    const launchpadAgent = await LaunchpadAgent.deploy(
      await factory.getAddress(),
      agent.address
    );
  ...
  describe("Deployment", function () {
    it("Should set the factory address", async function () {
      const { factory, launchpadAgent } = await loadFixture(
        deployLaunchpadAgentFixture
      );
      expect(await launchpadAgent.factory()).to.equal(
        await factory.getAddress()
      );
    });

    it("Should set the agent address", async function () {
      const { launchpadAgent, agent } = await loadFixture(
        deployLaunchpadAgentFixture
      );
    ...
    it("Should set the owner", async function () {
      const { launchpadAgent, deployer } = await loadFixture(
        deployLaunchpadAgentFixture
      );
  ...
  describe("Twitter Handle Registration", function () {
    it("Should register twitter handle", async function () {
      const { launchpadAgent, user1 } = await loadFixture(
        deployLaunchpadAgentFixture
      );
      const twitterHandle = "user1_twitter";

      await launchpadAgent.connect(user1).registerTwitterHandle(twitterHandle);
      expect(await launchpadAgent.twitterToAddress(twitterHandle)).to.equal(
        user1.address
      );
    });
  ...
  describe("Token Credits", function () {
    it("Should allow users to buy token credits", async function () {
      ...
      await launchpadAgent
        .connect(user1)
        .buyTokenCredits({ value: creditAmount });
      expect(await launchpadAgent.getUserTokenCredits(user1.address)).to.equal(
        creditAmount
      );
    });
  ...
  describe("Token Creation", function () {
    it("Should allow agent to create token for registered user", async function () {
      const { launchpadAgent, factory, agent, user1 } = await loadFixture(
        deployLaunchpadAgentFixture
      );
      const twitterHandle = "user1_twitter";
    ...
    it("Should not allow non-agent to create token", async function () {
      const { launchpadAgent, user1, user2 } = await loadFixture(
        deployLaunchpadAgentFixture
      );
      const twitterHandle = "user1_twitter";
    ...
    it("Should not create token if user has insufficient credits", async function () {
      const { launchpadAgent, agent, user1 } = await loadFixture(
        deployLaunchpadAgentFixture
      );
      const twitterHandle = "user1_twitter";
  ...
  describe("Token Creation via Address", function () {
    ...
    it("Should not create token via address if user has insufficient credits", async function () {
      ...
      await launchpadAgent
        .connect(user1)
        .buyTokenCredits({ value: insufficientAmount });
    ...
    it("Should deduct correct amount of credits when creating token via address", async function () {
      ...
      await launchpadAgent
        .connect(user1)
        .buyTokenCredits({ value: initialCredits });
      ...
      const remainingCredits = await launchpadAgent.getUserTokenCredits(
        user1.address
      );
